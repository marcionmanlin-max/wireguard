...
import { Zap, Lock, Eye, EyeOff, AlertTriangle, Smartphone, Download, Sun, Moon, Monitor, User, Shield } from 'lucide-react'
import { api } from '../utils/api'
import { useTheme } from '../contexts/ThemeContext'

export default function Login({ onLogin }) {
  const [tab, setTab] = useState('subscriber') // 'admin' | 'subscriber'
  ...
  const handleSubscriberSubmit = async (e) => {
    e.preventDefault()
    if (!email.trim() || !subPassword.trim()) return
    setLoading(true)
    setError('')
    try {
      const res = await fetch('/dns/api/subscribe/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: email.trim(), password: subPassword }),
      })
      const data = await res.json()
      if (!res.ok || data.error) throw new Error(data.error || 'Login failed')
      window.location.href = `/dns/client/?token=${encodeURIComponent(data.token)}`
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-dark-950 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm">
        ...
        {/* Tab switcher */}
        <div className="flex mb-4 bg-dark-900 border border-dark-700 rounded-xl p-1 gap-1">
          <button
            onClick={() => switchTab('subscriber')}
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'subscriber' ? 'bg-primary-400/15 text-primary-400 border border-primary-400/30' : 'text-dark-400 hover:text-dark-200'}`}
          >
            <User className="w-4 h-4" /> Subscriber
          </button>
          <button
            onClick={() => switchTab('admin')}
            className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'admin' ? 'bg-primary-400/15 text-primary-400 border border-primary-400/30' : 'text-dark-400 hover:text-dark-200'}`}
          >
        ...
        {tab === 'subscriber' && (
          <form onSubmit={handleSubscriberSubmit} className="bg-dark-900 border border-dark-700 rounded-2xl p-6 shadow-2xl">
            <div className="flex items-center gap-2 mb-5">
              <User className="w-4 h-4 text-primary-400" />
              <h2 className="text-white font-semibold">Subscriber Login</h2>
            </div>
            ...
            <div className="mb-4">
              <input
                ...
                autoFocus
                className="w-full px-4 py-3 bg-dark-800 border border-dark-600 rounded-xl text-white placeholder-dark-500 focus:outline-none focus:border-primary-400/50 focus:ring-1 focus:ring-primary-400/20 transition-all text-sm"
              />
            </div>
            ...
            <div className="relative mb-5">
              ...
              >
                {showSubPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              </button>
            </div>

            <button
              type="submit"
              disabled={loading || !email.trim() || !subPassword.trim()}
        ...
        {/* Admin Login Card */}
        {tab === 'admin' && (
          <form onSubmit={handleAdminSubmit} className="bg-dark-900 border border-dark-700 rounded-2xl p-6 shadow-2xl">
            <div className="flex items-center gap-2 mb-5">
              <Lock className="w-4 h-4 text-primary-400" />
              <h2 className="text-white font-semibold">Admin Authentication</h2>
            </div>
